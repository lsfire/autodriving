...
public class POMDP implements Serializable {
	...
	public double O(int iAction, BeliefState bsBelief, int iObservation) {
		...
		for (state = 0; state < m_cStates; state++) {
			totalProbability += bsBelief.valueAt(state)
					* O(iAction, state, iObservation);
		}
		return totalProbability;

	}

	public double O(int iStartState, int iAction, int iEndState,
			int iObservation) {
	...
	public int observe(BeliefState bs, int iAction) {
		...
		for (iObservation = 0; (iObservation < m_cObservations)
				&& (dProb > 0.0); iObservation++) {
	...
	public double computeAverageDiscountedReward(int cTests,
		...
		return computeAverageDiscountedReward(cTests, cMaxStepsToGoal, policy,
				true, ExecutionProperties.useHighLevelMultiThread()
	...
	public double computeAverageDiscountedReward(int cTests,
		...
		if (bUseMultiThread) {
			LinearValueFunctionApproximation vValueFunction = policy
					.getValueFunction();
			for (iThread = 0; iThread < cThreads; iThread++) {
				aTrials[iThread] = new ComputeDiscountedReward(this,
						cMaxStepsToGoal, vValueFunction, cTests / cThreads);
		...
		if (bUseMultiThread) {
			...
			for (iTest = 0; (iTest < cTests) && (dStandardError > 0.01 * dADR); iTest++) {
				...
				if (iTest >= 50 && iTest % 10 == 0) {
					...
					dStdev = Math
							.sqrt((dSumSquares - (iTest + 1) * dADR * dADR)
									/ iTest);
			...
			Logger.getInstance().logln(
					"\tAvg. steps per trial " + m_cSteps / iTest);
		}
	...
	public double computeAverageDiscountedRewardParticleFiltering(int cTests,
			int cMaxStepsToGoal, PolicyStrategy policy,
			boolean bOutputMessages, boolean bUseMultiThread) {
		countStatistics(false);
		...
		for (iTest = 0; (iTest < cTests) && (dStandardError > 0.05 * dADR); iTest++) {
			...
			dDiscountedReward = computeDiscountedRewardWithParticleFiltering(
					cMaxStepsToGoal, policy, cParticles);
	...
	public double computeAverageDiscountedRewardParticleFilteringImportanceSampling(
			int cTests, int cMaxStepsToGoal, PolicyStrategy policy,
			boolean bOutputMessages, boolean bUseMultiThread) {
		countStatistics(false);
		...
		for (iTest = 0; (iTest < cTests) && (dStandardError > 0.01 * dADR); iTest++) {
			dDiscountedReward = computeDiscountedRewardParticleFilteringImportanceSampling(
					cMaxStepsToGoal, policy, cParticles);
			dSumSquares += (dDiscountedReward * dDiscountedReward);
			dSumDiscountedRewards += dDiscountedReward;
	...
	private double computeExpectation(Vector<Double> vPrLogs,
	...
	private double computeVariance(Vector<Double> vPrLogs,
			Vector<Double> vValues, double dExpectation) {
		int iElement = 0, cElements = vPrLogs.size();
		Vector<Double> vLogs = new Vector<Double>();
	...
	protected double computeMDPAverageDiscountedReward(int cTests,
			int cMaxStepsToGoal) {
		...
		int iTest = 0;
		long cCurrentMilliseconds = 0, iStartTime = System.currentTimeMillis(), iEndTime = 0;
		boolean bCacheBeliefState = getBeliefStateFactory().cacheBeliefStates(
				false);

		for (iTest = 0; iTest < cTests; iTest++) {
			dSumDiscountedRewards += computeMDPDiscountedReward(
					cMaxStepsToGoal, vfMDP, false, null);
		}

		iEndTime = System.currentTimeMillis();
		cCurrentMilliseconds = (iStartTime - iEndTime) / 1000;

		String sInfo = "Status: #tests = " + cTests + ": " + " Time: "
				+ cCurrentMilliseconds + " Reward: " + dSumDiscountedRewards
				/ cTests + g_sNewline;
	...
	protected double computeDiscountedReward(int cMaxStepsToGoal,
			...
			int[] aiActionCount) {
		return computeDiscountedReward(cMaxStepsToGoal, policy,
				vObservedBeliefPoints, false, aiActionCount);
	}
	...
	public double computeDiscountedRewardWithParticleFiltering(
			int cMaxStepsToGoal, PolicyStrategy policy, int cMaxBeliefs) {
		...
		vCurrentBeliefs.add(new Pair<BeliefState, Double>(
				getBeliefStateFactory().getInitialBeliefState(), 1.0));
		...
		for (iStep = 0; (iStep < cMaxStepsToGoal) && !bDone; iStep++) {
			...
			for (Entry<BeliefState, Double> e : vCurrentBeliefs) {
				...
				if (bsCurrentBelief.isDeterministic()
						&& isTerminalState(bsCurrentBelief
				...
				for (iObservation = 0; iObservation < m_cObservations; iObservation++) {
					...
					if (bsNext != null) {
						...
						if (!bFound)
							vNextBeliefs.add(new Pair<BeliefState, Double>(
									bsNext, dWeight * dPrOGivenA));

						dSumNextWeights += dWeight * dPrOGivenA;

						dCurrentReward = R(bsCurrentBelief, iAction, bsNext);
						if (dCurrentReward != 0.0) {
							dCurrentStepDiscountedReward += dWeight
									* dPrOGivenA * dCurrentReward
									* dDiscountFactor;
						}
					}
				}
			...
			if (dSumTerminalBeliefs > 0.99) {
				...
				vCurrentBeliefs = reduceBeliefSet(vNextBeliefs, cMaxBeliefs,
						policy, comp);
				vNextBeliefs.clear();
			}
	...
	public double computeDiscountedRewardWithParticleFilteringImportanceSampling(
			int cMaxStepsToGoal, PolicyStrategy policy, int cMaxBeliefs) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		double dCurrentStepDiscountedReward = 0.0, dSumWeights = 0.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		...
		for (iStep = 0; (iStep < cMaxStepsToGoal) && !bDone; iStep++) {
			...
			for (Entry<BeliefState, Double> e : vCurrentBeliefs) {
				...
				if (bsCurrentBelief.isDeterministic()
					...
					vNextBeliefs.add(new Pair<BeliefState, double[]>(
							bsCurrentBelief, adNextBeliefWeights));
				} else {
					iAction = policy.getAction(bsCurrentBelief);
					...
					for (iObservation = 0; iObservation < m_cObservations; iObservation++) {
						...
						if (bsNext != null) {
							dPrOGivenA = bsCurrentBelief.probabilityOGivenA(
									iAction, iObservation);
							dValue = policy.getMDPValueFunction().getValue(
									bsNext)
									+ dValueOffset;
	...
	private Vector<Pair<BeliefState, Double>> reduceBeliefSet(
			Vector<Pair<BeliefState, double[]>> vNextBeliefs, int cMaxSamples,
			BeliefStateComparator comp) {
		...
		for (Pair<BeliefState, double[]> pOrg : vNextBeliefs) {
			...
			dValue = adBeliefWeights[0] * adBeliefWeights[1]
					* adBeliefWeights[2];
			dSumWeights += dValue;
		}
		...
		for (iSample = 0; iSample < cMaxSamples; iSample++) {
			...
			for (Pair<BeliefState, double[]> pOrg : vNextBeliefs) {
				...
				dRand -= dValue;
				if (dRand <= 0.0) {
					vSample.add(new Pair<BeliefState, double[]>(pOrg.getKey(),
							adBeliefWeights));
					break;
				}
			}
		...
		for (Pair<BeliefState, double[]> pSample : vSample) {
			...
			if (!bFound)
				vReduced.add(new Pair<BeliefState, Double>(pSample.getKey(),
						dValue));
			dSumWeights += dValue;
		}
		for (Pair<BeliefState, Double> pNew : vReduced) {
	...
	private Vector<Pair<BeliefState, Double>> reduceBeliefSet(
			Vector<Pair<BeliefState, Double>> vNextBeliefs, int cMaxSamples,
			PolicyStrategy policy, BeliefStateComparator comp) {
			...
			for (iSample = 0; iSample < cMaxSamples; iSample++) {
				...
				for (Pair<BeliefState, Double> pOrg : vNextBeliefs) {
					...
					if (dRand <= 0.0) {
						...
						if (!bFound) {
							vReduced.add(new Pair<BeliefState, Double>(pOrg
									.getKey(), 1.0));
						}
						dSumWeights++;
						break;
	...
	public double computeDiscountedRewardI(int cMaxStepsToGoal,
			PolicyStrategy policy, Vector<BeliefState> vObservedBeliefPoints,
			boolean bExplore, int[] aiActionCount) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		...
		for (iStep = 0; (iStep < cMaxStepsToGoal) && !bDone; iStep++) {
			...
			if (bsCurrentBelief.isDeterministic())
				bDone = endADR(bsCurrentBelief.getDeterministicIndex(),
						dCurrentReward);
	...
	/**
	 * 模拟执行cMaxStepsToGoal步，按指定的策略和起始状态概率及各种函数，计算出折扣回报
	 * R + yR + y2R + y3R + ...
	 * 
	 * @param cMaxStepsToGoal
	 * @param policy
	...
	public double computeDiscountedRewardParticleFilteringImportanceSamplingII(
			int cMaxStepsToGoal, PolicyStrategy policy, int cParticles) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		...
		for (iParticle = 0; iParticle < cParticles; iParticle++)
			aCurrent[iParticle] = getBeliefStateFactory()
		...
		for (iStep = 0; iStep < cMaxStepsToGoal && !bDone; iStep++) {
			...
			for (iParticle = 0; iParticle < cRemianingParticles; iParticle++) {
				...
				if (bsCurrent.isTerminalBelief()) {
					...
					Number[] anSampling = policy.getMDPValueFunction()
							.importanceSampling(iAction, bsCurrent);
					...
					dTrueProbability = bsCurrent.probabilityOGivenA(iAction,
							iObservation);
					...
					dCurrentReward = dWeight * R(bsCurrent, iAction, bsNext)
							/ cParticles;
					if (dCurrentReward > 0)
	...
	public double computeDiscountedRewardParticleFilteringImportanceSampling(
			int cMaxStepsToGoal, PolicyStrategy policy, int cParticles) {
		double dDiscountedReward = 0.0, dCurrentReward = 0.0, dDiscountFactor = 1.0;
		int iStep = 0, iAction = 0, iObservation = 0;
		BeliefState bsCurrentBelief = getBeliefStateFactory()
		...
		Number[] anSampling = policy.getMDPValueFunction()
				.importanceSamplingForStartState();
		...
		for (iParticle = 0; iParticle < cParticles; iParticle++) {
			vBeliefTrajectories.elementAt(0)[iParticle] = getBeliefStateFactory()
					.getInitialBeliefState();
			anSampling = policy.getMDPValueFunction()
					.importanceSamplingForStartState();
			iState = anSampling[0].intValue();
			vWeightTrajectories.elementAt(0)[iParticle] = probStartState(iState)
					/ anSampling[1].doubleValue();

			vTrueProbabilities.elementAt(0)[iParticle] = probStartState(iState);
			vImportanceProbabilities.elementAt(0)[iParticle] = anSampling[1]
					.doubleValue();
			vUnNormalizedWeighteds.elementAt(0)[iParticle] = probStartState(iState)
					/ anSampling[1].doubleValue();
			...
			if (cUnfinishedParticles > 0) {
				...
				for (iParticle = 0; iParticle < cUnfinishedParticles; iParticle++) {
					...
					if (vUnNormalizedWeighteds.elementAt(iStep)[iSample] > 0) {
						...
						anSampling = policy.getMDPValueFunction()
								.importanceSampling(iAction, iState);
						...
						dDiscountedReward += dWeight * dDiscountFactor
								* dCurrentReward;

						dTruePr = tr(iState, iAction, iNextState)
								* O(iAction, iNextState, iObservation);
						dImportancePr = anSampling[1].doubleValue()
								* O(iAction, iNextState, iObservation);
						...
						if (isTerminalState(iNextState)) {
							vFinalWeights.add(vUnNormalizedWeighteds
									.elementAt(iStep)[iSample]
									* (dTruePr / dImportancePr));
							vFinalDiscountedRewards.add(vDiscountedRewards
									.elementAt(iStep - 1)[iSample]
									+ dDiscountFactor * dCurrentReward);
							vUnNormalizedWeighteds.elementAt(iStep + 1)[iParticle] = 0.0;
						} else {
							bDone = false;
							vUnNormalizedWeighteds.elementAt(iStep + 1)[iParticle] = vUnNormalizedWeighteds
									.elementAt(iStep)[iSample]
									* (dTruePr / dImportancePr);
						}
						...
						vWeightTrajectories.elementAt(iStep + 1)[iParticle] = dWeight
								* (dTruePr / dImportancePr);
						...
						if (iStep > 0)
							vDiscountedRewards.elementAt(iStep)[iParticle] = vDiscountedRewards
									.elementAt(iStep - 1)[iSample]
									+ dDiscountFactor * dCurrentReward;
						else
							vDiscountedRewards.elementAt(iStep)[iParticle] = dDiscountFactor
									* dCurrentReward;
					}
					iSample++;
				}
				cUnfinishedParticles = cParticles
						- vFinalDiscountedRewards.size();
			}
		...
		for (iParticle = 0; iParticle < cUnfinishedParticles; iParticle++) {
			vFinalWeights
					.add(vUnNormalizedWeighteds.elementAt(iStep)[iParticle]);
			vFinalDiscountedRewards
					.add(vDiscountedRewards.elementAt(iStep - 1)[iParticle]);
		}
	...
	public double computeMDPDiscountedReward(int cMaxStepsToGoal,
			MDPValueFunction policy, boolean bExplore,
		...
		for (iStep = 0; (iStep < cMaxStepsToGoal) && !bDone; iStep++) {
			...
			if (false) {
				Logger.getInstance().logln(
						"Step " + iStep + ": s = " + getStateName(iState)
								+ " a = " + getActionName(iAction) + " o = "
								+ getObservationName(iObservation) + " s' = "
								+ getStateName(iNextState) + " reward = "
								+ dCurrentReward + " ADR = "
								+ round(dDiscountedReward, 3)
								+ " discount factor "
								+ round(dDiscountFactor, 3));
			}
			iState = iNextState;
			if (vBeliefPoints != null) {
	...
	private void addBackwardTransition(int iStartState, int iAction,
			int iEndState, double dTr) {
	}

	public Collection<Entry<Integer, Double>> getNonZeroBackwardTransitions(
			int iAction, int iEndState) {
	...
	public double R(BeliefState bsCurrent, int iAction, BeliefState bsNext) {
		Iterator<Entry<Integer, Double>> itNonZero = bsCurrent
				.getNonZeroEntries().iterator(), itNonZeroNext = null;
		...
		while (itNonZero.hasNext()) {
			...
			if (m_rtReward == RewardType.StateActionState) {
				...
				while (itNonZeroNext.hasNext()) {
					...
					dR += dBelief * dNextBelief
							* R(iState, iAction, iNextState);
				}
			}
		}
		return dR;
