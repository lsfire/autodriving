...
public class SingleExperiment {
	...
	public static void main( String[] args ) throws IOException{
		...
		if (args.length == 13)
		{
			sExperimentType = args[0]; 	// 试验类型，（目前只有Basic一种取值）
			sModelName = args[1];		// 数据集的名称
			maxRunningTime = Integer.parseInt(args[2]);		//最大运行时间
			numEvaluations = Integer.parseInt(args[3]);		//和maxRunningTime两者决定的值迭代的timePerEval
			numIndependentTrials = Integer.parseInt(args[4]);	// （PBVI算法确定的情况下）重复的次数
			collectionName = args[5];						// collection方法名称：GapMinCollection,FSVICollection,HSVICollection,PEMACollection,PBVICollection,RandomCollection
			orderingName = args[6];							// PerseusBackup, FullBackup, NewestPointsBackup
			numBeliefsPerStepOrg = Integer.parseInt(args[7]);	// 
			numBackupIterations = Integer.parseInt(args[8]);	//	imporveValueFunction()执行次数
			if (Boolean.parseBoolean(args[9]))				//是否允许反序
				bReversedBackupOrder = true;				
			else
				bReversedBackupOrder = false;				
			if (Boolean.parseBoolean(args[10]))				// expand信念点集过程中是否允许重复点的出现
				bAllowDuplicates = true;					
			else
				bAllowDuplicates = false;
			if (Boolean.parseBoolean(args[11]))				// 是否使用FIB算法
				bUseFIB = true;
			else
				bUseFIB = false;
			if (Boolean.parseBoolean(args[12]))		// 是否使用blind 策略
				useBlindPolicy = true;
			else
		...
		if( collectionName.equals( "PEMACollection"  ) ) 	//		PEMA， numBeliefPerStepOrg参数除以10
